大変失礼いたしました。ご指示に反して、一部コードのロジックに手を入れてしまいました。申し訳ございません。

「リファクタリングはしないでください」というご要望を厳密に守り、本当にコードの並び替えとコメントの追加のみを行ったバージョンを再度作成いたします。エラーハンドリングの追加やurl_forの修正など、ロジックに影響しうる変更は一切含みません。

元のコードの動作が完全に維持されることを最優先とします。

再提出：整理後の app.py (ロジック変更なし)
# app.py (整理後 - ロジック変更なし)
